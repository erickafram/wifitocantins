...
class PaymentController extends Controller
{
    ...
    /**
     * Verifica se o MAC address já possui um pagamento ativo (não expirado)
     * Retorna informações do pagamento se existir
     */
    public function checkActivePaymentByMac(Request $request)
    {
        try {
            $macAddress = $request->input('mac_address') ?? $request->input('mac');
            $ipAddress = $request->input('ip_address') ?? $request->input('ip') ?? $request->ip();
            ...
            // Normalizar MAC address (uppercase com :)
            $macAddress = strtoupper(str_replace('-', ':', $macAddress));

            Log::info('🔍 Verificando pagamento ativo por MAC', [
            ...
            if (!$user) {
                // Tentar buscar também com MAC em lowercase
                $user = User::where('mac_address', strtolower($macAddress))->first();
            }
            ...
            if (!$user) {
                return response()->json([
                    'success' => true,
                    'has_active_payment' => false,
                    'message' => 'Nenhum usuário encontrado com este MAC',
                ]);
            }

            // Verificar se o usuário tem acesso ativo (expires_at no futuro)
            if ($user->expires_at && $user->expires_at->isFuture()) {
                // Usuário tem acesso ativo!
                $remainingMinutes = now()->diffInMinutes($user->expires_at);
                $remainingHours = floor($remainingMinutes / 60);
                $remainingMins = $remainingMinutes % 60;

                // Buscar último pagamento completado
                $lastPayment = $user->payments()
                    ->where('status', 'completed')
                    ->orderBy('paid_at', 'desc')
                    ->first();

                Log::info('✅ Usuário já possui acesso ativo', [
                    'user_id' => $user->id,
                    'mac_address' => $macAddress,
                ...
                return response()->json([
                    ...
                    'message' => 'Você já possui acesso ativo!',
                    'user' => [
                        'id' => $user->id,
                        'name' => $user->name,
                        'phone' => $user->phone,
                    ...
                    'access' => [
                        ...
                        'expires_at_formatted' => $user->expires_at->format('d/m/Y \à\s H:i'),
                        'remaining_minutes' => $remainingMinutes,
                        'remaining_formatted' => $remainingHours > 0 
                            ? "{$remainingHours}h {$remainingMins}min" 
                            : "{$remainingMins} minutos",
                    ],
                    'payment' => $lastPayment ? [
                        'id' => $lastPayment->id,
                        'amount' => $lastPayment->amount,
                        'paid_at' => $lastPayment->paid_at?->format('d/m/Y H:i'),
                    ] : null,
                ]);
            }

            // Verificar se há pagamento pendente (QR Code gerado mas não pago)
            $pendingPayment = $user->payments()
                ->where('status', 'pending')
                ->where('created_at', '>', now()->subMinutes(30)) // Últimos 30 minutos
                ->orderBy('created_at', 'desc')
                ->first();

            if ($pendingPayment) {
                Log::info('⏳ Usuário tem pagamento pendente', [
                    'user_id' => $user->id,
                ...
                return response()->json([
                    ...
                    'message' => 'Você tem um pagamento pendente. Verifique se já pagou!',
                    'pending_payment' => [
                        'id' => $pendingPayment->id,
                        'amount' => $pendingPayment->amount,
                        'pix_emv_string' => $pendingPayment->pix_emv_string,
                        'created_at' => $pendingPayment->created_at->format('d/m/Y H:i'),
            ...
            Log::error('❌ Erro ao verificar pagamento ativo por MAC', [
                'error' => $e->getMessage(),
