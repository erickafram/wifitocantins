...
/interface wireless set wlan1 mode=ap-bridge ssid="Tocantins_WiFi_Express" \
    band=2ghz-b/g/n frequency=auto channel-width=20/40mhz-XX \
    wireless-protocol=802.11 security-profile=default

/interface wireless set wlan2 mode=ap-bridge ssid="Tocantins_WiFi_5G" \
    band=5ghz-a/n/ac frequency=auto channel-width=20/40/80mhz-XXXX \
    wireless-protocol=802.11 security-profile=default
...
/ip dhcp-server add name=hotspot-dhcp interface=lan-local address-pool=hotspot-pool \
...
/ip firewall filter add chain=forward action=accept \
    connection-state=established,related comment="Allow established connections"

/ip firewall filter add chain=forward action=accept \
    connection-state=new src-address=10.10.10.0/24 \
    comment="Allow hotspot users"

# Bloquear acesso direto sem autenticação
/ip firewall filter add chain=forward action=drop \
    src-address=10.10.10.0/24 \
    comment="Drop non-authenticated users"

# NAT para internet
/ip firewall nat add chain=srcnat out-interface=wan-starlink \
    action=masquerade comment="Internet NAT"
...
/user add name=api-tocantins password=TocantinsWiFi2024! group=full \
    comment="Usuario para API externa"
...
# Configurar DNS
/ip dns set servers=8.8.8.8,1.1.1.1 cache-size=2048KiB \
    max-concurrent-queries=100 max-concurrent-tcp-sessions=20

# =====================================================
# 9. REDIRECIONAMENTO CAPTIVE PORTAL
# =====================================================

# Configurar redirecionamento HTTP
/ip hotspot walled-garden ip add action=allow dst-address=0.0.0.0/0 \
    protocol=tcp dst-port=53,80,443 comment="Allow DNS and HTTP/HTTPS"
...
# Otimizar wireless
/interface wireless set wlan1 tx-power=20 tx-power-mode=default \
    distance=indoors country=brazil installation=indoor

/interface wireless set wlan2 tx-power=20 tx-power-mode=default \
    distance=indoors country=brazil installation=indoor
...
/system script add name=backup-automatico source={
    /system backup save name=("backup-tocantins-" . [/system clock get date])
    :log info "Backup automatico realizado"
}

# Agendar backup diário
/system scheduler add name=backup-diario start-time=03:00:00 \
    interval=1d on-event=backup-automatico
