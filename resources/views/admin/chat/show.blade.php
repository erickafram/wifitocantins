...
<div class="flex gap-6 h-[calc(100vh-160px)]">
    ...
    <div class="flex-1 flex flex-col bg-white rounded-2xl shadow-xl overflow-hidden">
        <!-- Header do Chat -->
        <div class="bg-gradient-to-r from-emerald-500 via-green-500 to-teal-600 p-4 text-white relative overflow-hidden">
            <div class="absolute top-0 right-0 w-40 h-40 bg-white/10 rounded-full -translate-y-1/2 translate-x-1/2"></div>
            <div class="relative flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    <a href="{{ route('admin.chat.index') }}" class="w-10 h-10 bg-white/20 hover:bg-white/30 rounded-xl flex items-center justify-center transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                        </svg>
                    </a>
                    <div class="w-14 h-14 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center text-2xl font-bold shadow-lg">
                        {{ strtoupper(substr($conversation->visitor_name, 0, 1)) }}
                    </div>
                    <div>
                        <h3 class="font-bold text-lg">{{ $conversation->visitor_name }}</h3>
                        <div class="flex items-center space-x-2 text-sm text-emerald-100">
                            <span class="w-2 h-2 rounded-full {{ $conversation->status === 'active' ? 'bg-green-300 animate-pulse' : ($conversation->status === 'pending' ? 'bg-yellow-300' : 'bg-gray-300') }}"></span>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <form action="{{ route('admin.chat.close', $conversation->id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="bg-white/20 hover:bg-white/30 px-4 py-2 rounded-xl text-sm font-medium transition-colors flex items-center space-x-2">
                    ...
                    <form action="{{ route('admin.chat.destroy', $conversation->id) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja excluir esta conversa?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="bg-red-500/80 hover:bg-red-500 px-3 py-2 rounded-xl text-sm transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Área de Mensagens -->
        <div id="messages-container" class="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-gray-50 to-white">
            <!-- Data inicial -->
            ...
            <div class="flex {{ $message->sender_type === 'admin' ? 'justify-end' : 'justify-start' }} chat-message-enter">
                ...
                <div class="flex items-end space-x-3 max-w-[70%]">
                    ...
                    <div class="bg-white rounded-2xl rounded-bl-md px-4 py-3 shadow-md border border-gray-100">
                        ...
                        <p class="text-xs text-gray-400 mt-2">{{ $message->created_at->format('H:i') }}</p>
                    </div>
                </div>
                @else
                <div class="max-w-[70%]">
                    <div class="bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-2xl rounded-br-md px-4 py-3 shadow-md">
                        ...
                        <div class="flex items-center justify-end space-x-2 mt-2">
                            @if($message->admin)
                            <span class="text-xs text-emerald-100">{{ $message->admin->name }}</span>
            ...
            <form id="reply-form" class="flex items-center space-x-3">
                ...
                <div class="flex-1 relative">
                    <input type="text" 
                           ...
                           class="w-full bg-gray-100 border-2 border-transparent rounded-xl px-5 py-4 focus:outline-none focus:border-emerald-500 focus:bg-white transition-all pr-12"
                           autocomplete="off"
                           required>
                </div>
                <button type="submit" 
                        class="w-14 h-14 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl font-medium hover:shadow-lg hover:shadow-emerald-500/30 transform hover:scale-105 transition-all flex items-center justify-center">
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden h-full">
            ...
            <div class="p-6 border-b bg-gradient-to-br from-gray-50 to-white">
                <div class="text-center">
                    <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-white text-3xl font-bold mx-auto shadow-lg">
                        {{ strtoupper(substr($conversation->visitor_name, 0, 1)) }}
                    </div>
                    <h4 class="font-bold text-gray-800 mt-4 text-lg">{{ $conversation->visitor_name }}</h4>
            ...
            <div class="p-6 border-t bg-gray-50">
                <a href="https://wa.me/55{{ preg_replace('/\D/', '', $conversation->visitor_phone) }}" 
                   target="_blank"
                   class="w-full bg-green-500 hover:bg-green-600 text-white py-3 rounded-xl font-medium transition-colors flex items-center justify-center space-x-2">
