...
<body class="font-jakarta min-h-screen floating-shapes relative overflow-x-hidden bg-gradient-to-br from-gray-50 via-purple-50/20 to-cyan-50/20">
    ...
    <!-- Modal de Acesso Ativo (Já Pagou) -->
    <div id="active-access-modal" class="fixed inset-0 bg-black bg-opacity-60 z-50 hidden backdrop-blur-sm">
        <div class="flex items-center justify-center h-full p-4">
            <div class="elegant-card rounded-3xl p-6 sm:p-8 w-full max-w-md animate-slide-up shadow-2xl relative overflow-hidden">
                ...
                <div class="text-center mb-6">
                    ...
                    <h3 class="text-2xl font-bold text-green-700">Você já tem acesso!</h3>
                    <p class="text-gray-600 mt-2">Seu pagamento foi confirmado</p>
                </div>

                <!-- Informações do acesso -->
                <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-5 mb-6 border border-green-200">
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            <span class="text-gray-600 flex items-center">
                                <span class="text-lg mr-2">👤</span> Nome:
                            </span>
                            <span id="active-user-name" class="font-semibold text-gray-800">-</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-gray-600 flex items-center">
                                <span class="text-lg mr-2">⏰</span> Expira em:
                            </span>
                            <span id="active-expires-at" class="font-semibold text-green-700">-</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-gray-600 flex items-center">
                                <span class="text-lg mr-2">⏱️</span> Tempo restante:
                            </span>
                            <span id="active-remaining" class="font-bold text-emerald-600 text-lg">-</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-yellow-50 border border-yellow-300 rounded-xl p-4 mb-6">
                    <p class="text-yellow-800 text-sm text-center">
                        <span class="font-bold">⚠️ Importante:</span> Não é necessário pagar novamente!<br>
                        Seu acesso está ativo e funcionando.
                    </p>
                </div>
                ...
                <div class="space-y-3">
                    <a href="https://www.google.com" 
                       class="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all text-center block">
                        🌐 NAVEGAR NA INTERNET
                    </a>
                    <button onclick="closeActiveAccessModal()" 
                            class="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-3 px-6 rounded-xl transition-all">
                        Fechar
                    </button>
                </div>

                <p class="text-center text-xs text-gray-500 mt-4">
                    🔒 Conexão segura • WiFi Tocantins Express
                </p>
            </div>
        </div>
    </div>

    <!-- Modal de Pagamento Pendente (com QR Code) -->
    <div id="pending-payment-modal" class="fixed inset-0 bg-black bg-opacity-60 z-50 hidden backdrop-blur-sm overflow-y-auto">
        <div class="flex items-center justify-center min-h-full p-4 py-8">
            <div class="elegant-card rounded-3xl p-6 sm:p-8 w-full max-w-md animate-slide-up shadow-2xl relative overflow-hidden">
                ...
                <div class="text-center mb-4">
                    ...
                    <h3 class="text-xl font-bold text-gray-800">Pagamento PIX Pendente</h3>
                    <p class="text-gray-600 text-sm mt-1">Escaneie o QR Code ou copie o código</p>
                </div>

                <!-- Valor -->
                <div class="bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl p-3 mb-4 text-center">
                    <p class="text-white text-sm">Valor a pagar</p>
                    <p id="pending-amount" class="text-white text-2xl font-bold">R$ -</p>
                </div>

                <!-- QR Code -->
                <div class="bg-white rounded-2xl p-4 mb-4 border-2 border-gray-200 shadow-inner">
                    <div id="pending-qr-container" class="flex justify-center items-center min-h-[200px]">
                        <div class="text-center text-gray-400">
                            <div class="animate-spin rounded-full h-8 w-8 border-4 border-gray-200 border-t-green-500 mx-auto mb-2"></div>
                            <p class="text-sm">Carregando QR Code...</p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Código Copia e Cola -->
                <div class="mb-4">
                    <p class="text-xs text-gray-500 text-center mb-2">📋 Código Copia e Cola</p>
                    <div class="relative">
                        <input type="text" 
                               id="pending-pix-code" 
                               readonly 
                               class="w-full bg-gray-100 border border-gray-300 rounded-lg px-3 py-2 text-xs text-gray-600 pr-20 truncate"
                               value="Carregando...">
                        <button onclick="copyPendingPixCode()" 
                                id="copy-pending-pix-btn"
                                class="absolute right-1 top-1 bg-green-500 hover:bg-green-600 text-white text-xs font-bold px-3 py-1 rounded-md transition-all">
                            📋 COPIAR
                        </button>
                    </div>
                </div>
                ...
                <div class="bg-yellow-50 border border-yellow-300 rounded-xl p-3 mb-4">
                    <p class="text-yellow-800 text-xs text-center">
                        <span class="font-bold">⚠️ Atenção:</span> Se você já pagou, aguarde alguns segundos.<br>
                        A liberação é automática após confirmação do banco.
                    </p>
                </div>

                <!-- Timer -->
                <div class="text-center mb-4">
                    <p class="text-gray-500 text-sm">⏱️ Gerado em: <span id="pending-created-at" class="font-semibold">-</span></p>
                </div>
                ...
                <div class="space-y-2">
                    <button onclick="checkPaymentStatus()" 
                            ...
                            class="w-full bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-bold py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all text-sm">
                        🔄 VERIFICAR SE JÁ PAGUEI
                    </button>
                    <div class="grid grid-cols-2 gap-2">
                        <button onclick="closePendingPaymentModal(); openRegistrationModal();" 
                                class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-2 px-4 rounded-xl transition-all text-xs">
                            📱 Novo QR Code
                        </button>
                        <button onclick="closePendingPaymentModal()" 
                                class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-2 px-4 rounded-xl transition-all text-xs">
                            ✕ Fechar
                        </button>
                    </div>
                ...
                <p class="text-center text-xs text-gray-400 mt-3">
                    💡 O acesso é liberado automaticamente em até 30 segundos após o pagamento
                </p>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // Variáveis globais para armazenar MAC e IP da URL
        window.MIKROTIK_MAC = null;
        window.MIKROTIK_IP = null;
        window.PENDING_PAYMENT_DATA = null;

        // Extrair MAC e IP da URL (vem do MikroTik)
        function extractMikrotikParams() {
            const urlParams = new URLSearchParams(window.location.search);
            window.MIKROTIK_MAC = urlParams.get('mac') || urlParams.get('mikrotik_mac') || urlParams.get('client_mac');
            window.MIKROTIK_IP = urlParams.get('ip') || urlParams.get('mikrotik_ip') || urlParams.get('client_ip');
            
            console.log('📱 Parâmetros MikroTik extraídos:', {
                mac: window.MIKROTIK_MAC,
                ip: window.MIKROTIK_IP
            });
        }

        // Verificar se usuário já tem pagamento ativo
        async function checkActivePayment() {
            if (!window.MIKROTIK_MAC) {
                console.log('⚠️ MAC não disponível na URL, pulando verificação');
                return;
            }

            try {
                console.log('🔍 Verificando pagamento ativo para MAC:', window.MIKROTIK_MAC);
                
                const response = await fetch('/api/payment/check-active?' + new URLSearchParams({
                    mac_address: window.MIKROTIK_MAC,
                    ip_address: window.MIKROTIK_IP || ''
                }));
                ...
                if (data.success) {
                    if (data.has_active_payment) {
                        // Usuário já tem acesso ativo!
                        showActiveAccessModal(data);
                    } else if (data.has_pending_payment) {
                        // Usuário tem pagamento pendente
                        showPendingPaymentModal(data.pending_payment);
                    }
                }
            } catch (error) {
                console.error('❌ Erro ao verificar pagamento ativo:', error);
            }
        }

        // Mostrar modal de acesso ativo
        function showActiveAccessModal(data) {
            const modal = document.getElementById('active-access-modal');
            if (!modal) return;

            // Preencher dados
            document.getElementById('active-user-name').textContent = data.user?.name || 'Usuário';
            document.getElementById('active-expires-at').textContent = data.access?.expires_at_formatted || '-';
            document.getElementById('active-remaining').textContent = data.access?.remaining_formatted || '-';

            // Mostrar modal
            modal.classList.remove('hidden');
            document.body.style.overflow = 'hidden';

            console.log('✅ Modal de acesso ativo exibido');
        }

        // Fechar modal de acesso ativo
        function closeActiveAccessModal() {
            const modal = document.getElementById('active-access-modal');
            if (modal) {
        ...
        // Mostrar modal de pagamento pendente com QR Code
        function showPendingPaymentModal(paymentData) {
            const modal = document.getElementById('pending-payment-modal');
            if (!modal) return;

            window.PENDING_PAYMENT_DATA = paymentData;

            // Preencher dados
            document.getElementById('pending-amount').textContent = 'R$ ' + parseFloat(paymentData.amount).toFixed(2).replace('.', ',');
            document.getElementById('pending-created-at').textContent = paymentData.created_at || '-';

            // Preencher código PIX
            const pixCode = paymentData.pix_emv_string || '';
            document.getElementById('pending-pix-code').value = pixCode;

            // Gerar QR Code
            const qrContainer = document.getElementById('pending-qr-container');
            if (pixCode) {
                // Usar API externa para gerar QR Code a partir do EMV string
                const qrImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(pixCode)}`;
                qrContainer.innerHTML = `
                    <img src="${qrImageUrl}" 
                         alt="QR Code PIX" 
                         class="w-48 h-48 mx-auto rounded-lg shadow-md"
                         onerror="this.parentElement.innerHTML='<p class=\\'text-red-500 text-sm\\'>Erro ao carregar QR Code</p>'"
                    >
                `;
                console.log('📱 QR Code gerado para pagamento pendente');
            } else {
                qrContainer.innerHTML = `
                    <div class="text-center text-gray-500">
                        <p class="text-sm">⚠️ QR Code não disponível</p>
                        <p class="text-xs mt-1">Use o código copia e cola abaixo</p>
                    </div>
                `;
            }
            ...
            console.log('⏳ Modal de pagamento pendente exibido', paymentData);
        }

        // Copiar código PIX pendente
        function copyPendingPixCode() {
            const pixCodeInput = document.getElementById('pending-pix-code');
            const copyBtn = document.getElementById('copy-pending-pix-btn');
            
            if (!pixCodeInput || !pixCodeInput.value || pixCodeInput.value === 'Carregando...') {
        ...
        // Fechar modal de pagamento pendente
        ...
        // Verificar status do pagamento pendente
        async function checkPaymentStatus() {
            if (!window.PENDING_PAYMENT_DATA) {
            ...
            try {
                const response = await fetch('/api/payment/pix/status?' + new URLSearchParams({
                    transaction_id: window.PENDING_PAYMENT_DATA.transaction_id
                }));
                ...
                if (data.success && data.status === 'completed') {
                    ...
                    alert('✅ Pagamento confirmado! Seu acesso está sendo liberado...');
                    
                    // Recarregar página para verificar acesso
                    setTimeout(() => {
                    ...
                    alert('ℹ️ Status: ' + (data.message || 'Aguardando pagamento'));
                }
            } catch (error) {
        ...
        // Abrir modal de registro (função auxiliar)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            extractMikrotikParams();
            
            // Verificar pagamento ativo após pequeno delay (para garantir que tudo carregou)
            setTimeout(() => {
                checkActivePayment();
            }, 500);
        });
